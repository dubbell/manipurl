...
def project_text(text_to_project):
    ...
    return (text_features / text_features.norm(p=2, dim=-1, keepdim=True)).squeeze()
...
def project_image(image_to_project):
    ...
    with torch.no_grad():
        image_features = model.get_image_features(**image_token)

    return (image_features / image_features.norm(p=2, dim=-1, keepdim=True)).squeeze()